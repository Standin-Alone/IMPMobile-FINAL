...
import {View,Text, StyleSheet,FlatList,Image,Modal,ScrollView, TouchableOpacity} from 'react-native';
...
// render card in flatlist
 renderItem = (item, index) => {
    ...
    item.name == "Valid ID" &&
      ...
      <View>
        <Text style={styles.title}> <FontAwesomeIcon name="info-circle" onPress = {()=>this.setState({showValidIdList:true})}color={Colors.blue_green} size={25}/> {item.name} <Text style={[styles.title,{color:Colors.danger}]}>*</Text></Text>
        <Button
    ...
    item.name == "Valid ID" ? (
      <View>
        <Text style={styles.title}> <FontAwesomeIcon name="info-circle"   

        color={Colors.blue_green} size={25}/> {item.name} </Text>
    ...
    return (
      <View  style={styles.container}>        
        ...
        <Modal
          animationType="slide"
          transparent={true}
          visible={this.state.showValidIdList}
          onRequestClose={() =>this.setState({showValidIdList:false})}
          
          style={{ margin:0,flexDirection:'flex-end' }}
          
        > 
          <View style={styles.list_of_valid_id_modal}>
              {/* content */} 
              <View style={styles.list_of_valid_id_modal_content}>
                  <Text style={styles.list_of_valid_id_title }>List of Valid ID</Text>
                  <TouchableOpacity style={{ alignItems:'flex-end',right:(Layout.width / 100) * 5 }} onPress={()=>this.setState({showValidIdList:false})}>
                      <FontAwesomeIcon name="close" color={Colors.danger} size={25}/>                  
                  </TouchableOpacity>
                  <ScrollView style={{top:(Layout.height / 100) * 5}}>
                    
                  <View style={{ flexDirection:'row',left:20,width:(Layout.width / 100) * 90}}>

                    <View>
                      <Text style={styles.valid_ids}>
                      ●	Alien Certification of Registration (ACR) {"\n"}
                      ●	Armed Forces of the Philippines (AFP) ID {"\n"}
                      ●	Association-issued ID {"\n"}
                      ●	Barangay Certification {"\n"}
                      ●	DA-issued ID {"\n"} 
                      ●	Department of Labor and Employment (DOLE) Visa {"\n"}
                      ●	Department of Labor and Employment (DSWD)-issued ID {"\n"}
                      ●	DSWD Certificate {"\n"}
                      ●	Driver’s License {"\n"} 
                      ●	Employment ID (Government and Private) {"\n"}
                      ●	Firearm License Card {"\n"} 
                      ●	Government Service Insurance System (GSIS) e-Card Plus {"\n"}
                      ●	Home Development Mutual Fund (HDMF) Card {"\n"}
                      ...
                      ●	Student ID (for non-voting/minor age students). Must be currently enrolled. Must present original and submit a clear copy of photo-bearing ID duly signed by the principal or head of the school {"\n"}
                      ●	Tax Identification Number (TIN) ID {"\n"}
                      ●	Unified Multi-Purpose ID (UMID) {"\n"}
                      ●	Voter’s ID {"\n"}
                      ●	Voter’s Certification {"\n"}
                      ●	Other IDs that may be approved by DA for use {"\n"}
...
const styles = StyleSheet.create({
  ...
  list_of_valid_id_modal_content: {
    width: (Layout.width / 100) * 100,
    height: (Layout.height / 100) * 50,    
    backgroundColor: 'white',
    overflow: 'hidden',
    paddingBottom:50,
